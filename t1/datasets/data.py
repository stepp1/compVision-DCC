...
def process_batch_threads(thr_index, ranges, filenames, labels, image_shape, tfr_filename,
    ...
    for idx in np.arange(ranges[thr_index][0], ranges[thr_index][1]):
        try:
            ...
            if count % 100 == 0:
                print(
                    '{} Thread {} --> processing {} of {} [{}, {}]'.format(datetime.now(), thr_index, count, batch_size,
                                                                           ranges[thr_index][0], ranges[thr_index][1]))
        except ValueError:
            n_reading_error = n_reading_error + 1
...
def create_tfrecords(config, _type, processFun=imgproc.resize_image):
    ...
    if (_type == 'train') or (_type == 'all'):
        ...
        if config.use_multithreads():
            ...
            training_mean = create_tfrecords_threads(filenames, labels, image_shape, tfr_filename, processFun,
                                                     n_threads)
        else:
