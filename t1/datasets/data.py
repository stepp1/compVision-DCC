...
def create_tfrecords_from_file(
    filenames, labels, image_shape, tfr_filename, process_function=imgproc.resize_image
):
    # create tf-records
...
def process_batch_threads(
    ...
    image_shape,
    tfr_filename,
    process_function=imgproc.resize_image,
):
    # create tf-records
...
def create_tfrecords_threads(
    filenames, labels, image_shape, tfr_filename, process_function, n_threads
):
    assert len(filenames) == len(labels)
    ...
    for idx in range(n_threads):
        ...
        if idx == 0:
            mean_image = np.reshape(
                np.fromfile(mean_filename, dtype=np.float32), image_shape
            )
        else:
            mean_image = mean_image + np.reshape(
                np.fromfile(mean_filename, dtype=np.float32), image_shape
            )
...
def create_tfrecords(config, _type, processFun=imgproc.resize_image):
    ...
    if (_type == "train") or (_type == "all"):
        ...
        if config.use_multithreads():
            ...
            training_mean = create_tfrecords_threads(
                filenames, labels, image_shape, tfr_filename, processFun, n_threads
            )
        else:
            tfr_filename = os.path.join(data_dir, "train.tfrecords")
            training_mean = create_tfrecords_from_file(
                filenames, labels, image_shape, tfr_filename, processFun
            )
    ...
    if (_type == "test") or (_type == "all"):
        ...
        if config.use_multithreads():
            ...
            create_tfrecords_threads(
                filenames, labels, image_shape, tfr_filename, processFun, n_threads
            )
        else:
            tfr_filename = os.path.join(data_dir, "test.tfrecords")
            create_tfrecords_from_file(
                filenames, labels, image_shape, tfr_filename, processFun
            )
    ...
    if with_augmentation:
        ...
        if prob < data_augmentation_prob:
            ...
            if prob < data_augmentation_prob * 0.5:
                ...
                image = tf.cast(
                    tf.image.resize(image, (input_shape[0], input_shape[1])), tf.uint8
                )
            else:
                image = tf.image.flip_left_right(image)
...
def parser_tfrecord_siamese(
    serialized_input, input_shape, mean_image, with_augmentation=False
):
    features = tf.io.parse_example(
        [serialized_input],
        features={
